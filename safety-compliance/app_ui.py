...
async def websocket_endpoint(websocket: WebSocket, uuid: str):
    ...
    # Generate unique session ID to avoid "User is already connected" errors
    ...
    try:
        while True:
            ...
            async with client.distiller(
                ...
                uuid=uuid,
                executor_dict={
                    "Detail Collector": make_human_input_func(uuid),
                },
            ) as dc:
                ...
                async for response in responses:
                    ...
                    if {"role", "content"}.issubset(response):
                        ...
                        # Detect HumanAgent prompt: the FlowSuperAgent sends
                        # the Detail Collector's prompt as its own message.
                        # Look for the prompt content asking for incident details.
                        is_human_prompt = (
                            "Detail Collector" in role
                            or "Detail Collector" in content
                            or (
                                "Incident Report Flow" in role
                                and any(kw in content.lower() for kw in [
                                    "gather", "collect", "date", "injuries",
                                    "what happened", "description of",
                                ])
                            )
                        )
                        if is_human_prompt and not human_relay_active:
