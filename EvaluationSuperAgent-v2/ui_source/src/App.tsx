...
function CollapsibleBubble({ content, isUser }: { content: string; isUser: boolean }) {
  const [expanded, setExpanded] = useState(false)
  const needsTruncation = !isUser && content.length > TRUNCATE_LENGTH
      ...
      <header className="header">
        <div className="header-inner">
          <div className="logo">
            ...
            <span className="logo-text">AI Refinery — Agent Evaluation</span>
          </div>
      ...
      <main className="chat-area">
        {messages.length === 0 ? (
          <div className="empty-state">
            ...
            <h2>Agent Evaluation Framework</h2>
            <p>Automated quality scoring for AI agents using predefined metrics, rubrics, and ground-truth comparisons.</p>
            <div className="example-queries">
              <p style={{fontSize: '13px', marginTop: '16px', color: '#9ca3af'}}>Try:</p>
              <p style={{fontSize: '13px', color: '#6b7280'}}>Type "evaluate" to score a single agent</p>
              <p style={{fontSize: '13px', color: '#6b7280'}}>Type "compare" to run a 2-agent comparison</p>
            </div>
          </div>
        ) : (
            ...
            {messages.map((msg) => (
              <div key={msg.id} className={`message-row ${msg.isUser ? 'user' : 'agent'}`}>
                ...
                <div className="message-content">
                  ...
                  <CollapsibleBubble content={msg.content} isUser={msg.isUser} />
                </div>
              </div>
            ))}
            {isWaiting && (
